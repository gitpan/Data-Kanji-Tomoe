...
This Perl module parses the kanji or hanzi data supplied by L<the Tomoe
"handwriting recognition engine"|/Tomoe>.

The data itself is not supplied with this module.

The parsing is based on L<XML::Parser>, and simply breaks the Tomoe
data into individual characters then calls back a routine supplied by
the user with the data for each character.
...
Start the object. A hash containing the name of the data file to be
parsed, under the key C<tomoe_data_file>, must be supplied.
...
this location. The most recent update of the software was on 17 May
2006, and the project is dormant. Those who have queries about Tomoe
might try L<the mailing list for the "Tegaki Handwriting Recognition
Project"|https://groups.google.com/group/tegaki-hwr>. This is a similar
project which some of the same people are involved in.
